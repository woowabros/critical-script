...
export default function DemoScreen({ variant }: Props): ReactElement {
  ...
  useEffect(() => {
    ...
    const pageStrings = DEMO_STRINGS[pageLocale]
    ...
    const run = async (): Promise<void> => {
      ...
      if (variant === 'with') {
        if (window.__demoHome === undefined) {
          setProblem(pageStrings.criticalMissing)
      ...
      setHome({
        ...data,
        ...pageStrings.home,
        heroImageUrl: `${throttled(data.heroImageUrl, throttle)}&fresh=${fresh}`,
      })
      setStage('ready')
    ...
    void run().catch(() => {
      if (!abandoned) setProblem(pageStrings.fetchFailed)
    })
