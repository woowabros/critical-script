...
At build time, `home.critical.ts` is compiled and minified by esbuild and inlined into the HTML as a `<script>` tag. See the [API Reference](./api-reference.md) for plugin options such as `define`, and [Use Cases](./use-cases.md) for common patterns.
