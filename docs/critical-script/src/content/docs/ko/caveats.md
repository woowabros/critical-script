...
- **인라인 스크립트는 HTML 로 렌더링되어야 남습니다**. import 한 컴포넌트가 React 렌더링 과정에서 `<script>` 태그를 만들기 때문에, 빌드 시점이나 서버에서 HTML 을 생성하는 환경에서 의미가 있습니다. framework mode 의 [react-router](https://github.com/remix-run/react-router) 와 [@tanstack/react-start](https://github.com/TanStack/router) 가 그런 환경입니다.
...
`critical.ts` 안의 import 를 줄이거나 코드를 단순화하세요. 큰 라이브러리 의존이 있다면 vanilla JS로 다시 작성하는 것을 권장합니다. 임계치를 늘리려면 plugin 옵션의 `outputSizeLimit` 값을 조정하세요. 다만 인라인 스크립트는 HTML 에 함께 실려 매 요청마다 전송되므로, 늘리더라도 가능한 한 작게 유지하는 편이 좋습니다.
