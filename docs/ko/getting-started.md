...
빌드 시 `home.critical.ts`가 esbuild로 컴파일·minify되어 HTML 의 `<script>` 태그로 인라인됩니다. `define` 등 플러그인 옵션은 [API 레퍼런스](./api-reference.md) 를, 활용 패턴은 [활용 사례](./use-cases.md) 를 참고하세요.
